...
juce::String runJackProcess(const juce::String& command)
{
  ...
  if (!process.start(command)) return {};
  return process.readAllProcessOutput();
}

bool jackClientHasPortKind(const juce::String& clientName, const juce::String& kind)
{
  if (clientName.isEmpty() || kind.isEmpty()) return false;
  return runJackProcess("jack_lsp").contains(clientName + ":" + kind);
}

juce::String preferJackCaptureDevice(const juce::StringArray& inputNames, const juce::String& outputName,
                                     const juce::String& currentInput)
{
  ...
  auto score = [&](const juce::String& name) -> int
  {
    ...
    if (outputName.isNotEmpty() && name == outputName && !hasCapture) s -= 200;

    if (name.containsIgnoreCase("monitor")) s -= 100;

    if (hasCapture && name.containsIgnoreCase("Analog")) s += 25;

    if (hasCapture && (name.containsIgnoreCase("IEC958") || name.containsIgnoreCase("Digital"))) s += 10;
    ...
    if (outputName.isNotEmpty())
    {
      const auto family = outputName.upToFirstOccurrenceOf(" Analog", false, false)
                              ...
                              .trim();
      if (family.isNotEmpty() && name.containsIgnoreCase(family)) s += 30;
    }

    if (name == currentInput) s += 5;
    return s;
  };
...
void reconnectJackAppInputsToCapture(const juce::String& appClient, const juce::String& inputClient)
{
  if (appClient.isEmpty() || inputClient.isEmpty()) return;
  if (!jackClientHasPortKind(inputClient, "capture")) return;
  ...
  for (auto& line : juce::StringArray::fromLines(allPorts))
  {
    ...
    if (port.startsWith(inputClient + ":capture")) capturePorts.addIfNotAlreadyThere(port);
    if (port.startsWith(appClient + ":in_")) appInputs.addIfNotAlreadyThere(port);
  }

  capturePorts.sort(true);
  appInputs.sort(true);
  if (capturePorts.isEmpty() || appInputs.isEmpty()) return;
  ...
  for (const auto& appIn : appInputs)
  {
    ...
    for (auto& raw : juce::StringArray::fromLines(connections))
    {
      ...
      if (src.isNotEmpty()) runJackProcess("jack_disconnect \"" + src + "\" \"" + appIn + "\"");
    }
  }

  const int n = juce::jmin(capturePorts.size(), appInputs.size());
  for (int i = 0; i < n; ++i) runJackProcess("jack_connect \"" + capturePorts[i] + "\" \"" + appInputs[i] + "\"");
}

void disconnectJackAppOutputFeedback(const juce::String& appClient)
{
  if (appClient.isEmpty()) return;
  ...
  for (auto& line : juce::StringArray::fromLines(runJackProcess("jack_lsp")))
  {
    const auto port = line.trim();
    if (port.startsWith(appClient + ":in_")) appInputs.addIfNotAlreadyThere(port);
    if (port.startsWith(appClient + ":out_")) appOutputs.addIfNotAlreadyThere(port);
  }

  if (appInputs.isEmpty()) return;
  ...
  for (const auto& appIn : appInputs)
  {
    ...
    for (auto& raw : juce::StringArray::fromLines(connectionsText))
    {
      ...
      if (fromSelfOut || fromMonitor) runJackProcess("jack_disconnect \"" + src + "\" \"" + appIn + "\"");
    }
  }
}
}  // namespace

void ensureJackUsesCaptureInput(juce::AudioDeviceManager& deviceManager, const juce::String& appClientName)
{
  ...
  // Ensure input device is set to a capture device
  ...
  // Ensure output device is set (JACK may leave it empty)
  if (setup.outputDeviceName.isEmpty() && outputNames.size() > 0)
  {
    // Prefer a playback device (not monitor, not capture)
    for (const auto& name : outputNames)
    {
      if (jackClientHasPortKind(name, "playback") && !jackClientHasPortKind(name, "monitor"))
      {
    ...
    // Fallback: any device with playback ports
    if (setup.outputDeviceName.isEmpty())
    {
      for (const auto& name : outputNames)
      {
        if (jackClientHasPortKind(name, "playback"))
        {
          setup.outputDeviceName = name;
          needsUpdate = true;
          break;
...
void prepareJackInputForTuning(juce::AudioDeviceManager& deviceManager, const juce::String& appClientName)
{
