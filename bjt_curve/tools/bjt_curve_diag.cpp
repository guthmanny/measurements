...
int main()
{
  ...
  auto families = bjt_curve::buildIcVsVceFamily(bjt_curve::BjtModelKind::IdealNpn,
                                                bjt_curve::CircuitKind::CommonEmitter, ibValues, 32, vceMax);
  auto overlay =
      bjt_curve::buildCommonEmitterDcLoadLineOverlay(bjt_curve::BjtModelKind::IdealNpn, ibValues, families, vceMax);
  ...
  for (size_t i = 0; i < ibValues.size(); ++i)
  {
    ...
    if (i < families.size() && !families[i].empty())
    {
      ...
      std::printf("curve: n=%zu  vce[0]=%.3f ic[0]=%.1fuA  vce[-1]=%.3f ic[-1]=%.1fuA\n", fam.size(), fam.front().first,
                  fam.front().second * 1e6f, fam.back().first, fam.back().second * 1e6f);
    }
    else
    {
