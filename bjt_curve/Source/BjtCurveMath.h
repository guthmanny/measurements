...
// Unified transistor model selection covering both NPN and PNP devices.
// AC128 is a PNP germanium transistor (negative Vbe/Vce polarity).
enum class BjtModelKind
{
  IdealNpn,
  Npn3904,
  Npn2222,
...
bool isPnp(BjtModelKind model) noexcept;
...
const nx_bjt_t& modelParams(BjtModelKind model) noexcept;
...
std::vector<std::pair<float, float>> buildCurve(BjtModelKind model, CircuitKind circuit, CurveKind kind, float ibAmps,
                                                int numPoints = 256, float vceMaxVolts = kDefaultMaxVce,
