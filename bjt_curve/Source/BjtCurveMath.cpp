...
double solveVbeCommonEmitter(double p, const bjt_common_emitter_coeffs_t& c) noexcept
{
    ...
    for (int it = 0; it < 50; ++it)
    {
        ...
        double ib = 0.0;
        double icRaw = 0.0;
        nx_bjt_em_currents_f64(c.bjt_Is, c.bjt_Vt, c.bf, c.br, vbe, vbc, &ib, &icRaw);
        const double ic = icRaw > c.ic_max ? c.ic_max : icRaw;
        double gPi = 0.0;
        double gM = 0.0;
        nx_bjt_em_derivs_dvbe_f64(c.bjt_Is, c.bjt_Vt, c.bf, c.br, vbe, vbc, &gPi, &gM);
        if (icRaw > c.ic_max)
            gM = 1.0e-12;
...
double ibAtCommonEmitterDc(const bjt_common_emitter_coeffs_t& c) noexcept
{
    ...
    double ib = 0.0;
    double icDummy = 0.0;
    nx_bjt_em_currents_f64(c.bjt_Is, c.bjt_Vt, c.bf, c.br, vbe, vbc, &ib, &icDummy);
    return ib;
}
...
double icAtCommonEmitterDc(const bjt_common_emitter_coeffs_t& c) noexcept
{
    ...
    double ibDummy = 0.0;
    double ic = 0.0;
    nx_bjt_em_currents_f64(c.bjt_Is, c.bjt_Vt, c.bf, c.br, vbe, vbc, &ibDummy, &ic);
    if (c.ic_max > 0.0 && ic > c.ic_max)
        return c.ic_max;
    return ic;
}
...
bool isCommonEmitterDcValid(const nx_bjt_common_emitter_config_t& config,
                            const bjt_common_emitter_coeffs_t& c) noexcept
{
    if (!std::isfinite(c.vb_dc) || !std::isfinite(c.vc_dc) || !std::isfinite(c.ve_dc))
        return false;
    const double vbe = c.vb_dc - c.ve_dc;
    const double vce = c.vc_dc - c.ve_dc;
    if (!std::isfinite(vbe) || !std::isfinite(vce))
        return false;
    if (vbe < -0.5 || vbe > 1.2)
        return false;
    if (vce < -0.2 || vce > config.vcc + 0.5)
        return false;
    return true;
}
...
{
    DcOperatingPoint q;
    if (!std::isfinite(vbe) || !std::isfinite(vce) || !std::isfinite(ib) || !std::isfinite(ic))
        return q;
    if (ib <= 0.0 && ic <= 0.0)
        return q;
    ...
    q.vce = static_cast<float>(juce::jmax(0.0, vce));
...
void sortSamplesByVbe(TabulateSamples& samples)
{
    ...
    std::sort(order.begin(), order.end(), [&samples](size_t a, size_t b)
              { return samples.vbe[a] < samples.vbe[b]; });
...
DcLoadLineCoeffs computeLoadLineFromDesignCore(const nx_bjt_common_emitter_config_t& config,
                                               double vb,
                                               double ib) noexcept
{
...
bool commonEmitterFixedConfig(nx_bjt_npn_model_e model, nx_bjt_common_emitter_config_t& configOut) noexcept
{
...
std::vector<std::pair<float, float>> buildLoadLineSegment(const DcLoadLineCoeffs& line,
                                                          double vceMin,
                                                          double vceMax) noexcept
{
    std::vector<std::pair<float, float>> segment;
    if (line.slope <= 0.0 || vceMax <= vceMin)
        return segment;
...
TabulateSamples tabulateCommonEmitter(nx_bjt_npn_model_e model)
{
    TabulateSamples samples;
    nx_bjt_common_emitter_config_t config{};
    nx_bjt_common_emitter_config_init(&config);
    config.bjt = model;
    ...
    for (size_t j = 0; j < tableEnd; ++j)
    {
        ...
        double ibEm = 0.0;
        double ic = 0.0;
        nx_bjt_em_currents_f64(coeffs.bjt_Is, coeffs.bjt_Vt, coeffs.bf, coeffs.br, vbe, vbc, &ibEm, &ic);
        if (coeffs.ic_max > 0.0 && ic > coeffs.ic_max)
            ic = coeffs.ic_max;
...
TabulateSamples tabulateFollower(nx_bjt_npn_model_e model)
{
    TabulateSamples samples;
    ...
    if (bjt_follower_tabulate_f32(&tableParams, kSampleRate, &config, &cache) != NX_SUCCESS
        || cache.tables == nullptr || cache.tables[0] == nullptr)
    {
        nl_circuits_cache_release_f32(&cache);
        return samples;
...
TabulateSamples tabulateFollowerOut(nx_bjt_npn_model_e model)
{
    TabulateSamples samples;
...
bool ibValuesEqual(const std::vector<float>& a, const std::vector<float>& b) noexcept
{
    ...
    {
        if (!juce::approximatelyEqual(a[i], b[i]))
            return false;
    }
    return true;
}

TabulateSamples tabulateCircuit(nx_bjt_npn_model_e model, CircuitKind circuit)
{
    {
        std::lock_guard<std::mutex> lock(curveCacheMutex());
        auto& cache = tabulateCache();
        if (cache.valid && cache.model == model && cache.circuit == circuit)
...
std::vector<std::pair<float, float>> buildTransferCurveFromEm(nx_bjt_npn_model_e model,
                                                              CircuitKind circuit,
                                                              CurveKind kind,
                                                              int numPoints,
                                                              float vbeMaxVolts)
{
    ...
    if (numPoints < 2 || kind == CurveKind::IcVsVce)
        return points;

    const uint32_t idx = static_cast<uint32_t>(model) & 0xFFU;
    const nx_bjt_t& bjt =
        nx_bjt_npn_models[idx < (static_cast<uint32_t>(NX_BJT_MODEL_COUNT) & 0xFFU) ? idx : 0U];
    ...
    for (int i = 0; i < numPoints; ++i)
    {
        ...
        const double vbe = minVbe + t * (maxVbe - minVbe);
        const double vbc = vbe - vceBias;
        double ib = 0.0;
        double ic = 0.0;
        nx_bjt_em_currents_f64(isVal, vtN, bf, br, vbe, vbc, &ib, &ic);
...
double solveVbeForConstantIb(const nx_bjt_t& bjt, double targetIb, double vce) noexcept
{
    const double isVal = static_cast<double>(bjt.Is);
    const double vtN = static_cast<double>(bjt.Vt) * static_cast<double>(bjt.N);
    const double bf = static_cast<double>(bjt.Bf);
    ...
    double vbe = vtN * std::log(1.0 + juce::jmax(1.0e-30, targetIb) * juce::jmax(bf, 1.0) / juce::jmax(isVal, 1.0e-30));
    vbe = juce::jlimit(-0.5, 1.2, vbe);
    ...
    for (int it = 0; it < 60; ++it)
    {
        ...
        if (std::abs(f) < 1.0e-15 * juce::jmax(1.0, targetIb) || std::abs(f) < 1.0e-18)
            break;
        ...
        double dibDvbc = 0.0;
        double dicDvbc = 0.0;
        nx_bjt_em_derivs_dvbe_f64(isVal, vtN, bf, br, vbe, vbc, &dibDvbe, &dicDvbe);
        nx_bjt_em_derivs_dvbc_f64(isVal, vtN, bf, br, vbe, vbc, &dibDvbc, &dicDvbc);
        // Vbc = Vbe − Vce ⇒ dIb/dVbe = ∂Ib/∂Vbe + ∂Ib/∂Vbc
        const double dF = dibDvbe + dibDvbc;
        if (std::abs(dF) < 1.0e-30)
            break;
...
std::vector<std::pair<float, float>> buildCommonEmitterOutputCurve(nx_bjt_npn_model_e model,
                                                                    double targetIb,
                                                                    int numPoints,
                                                                    float vceMaxVolts)
{
    std::vector<std::pair<float, float>> points;
    if (numPoints < 2 || targetIb <= 0.0)
        return points;
    ...
    auto appendAtVce = [&](double vce)
    {
        vce = juce::jlimit(0.0, vceMax, vce);
        const double vbe = solveVbeForConstantIb(bjt, targetIb, vce);
        const double vbc = vbe - vce;
        double ib = 0.0;
        double ic = 0.0;
        nx_bjt_em_currents_f64(isVal, vtN, bf, br, vbe, vbc, &ib, &ic);
        ...
        points.emplace_back(static_cast<float>(vce), static_cast<float>(juce::jmax(0.0, ic)));
    };
...
{
    DcOperatingPoint q{};
    if (curve.size() < 2 || line.slope <= 0.0)
        return q;
...
{
    const auto index = static_cast<uint32_t>(model) & 0xFFU;
    if (index >= static_cast<uint32_t>(NX_BJT_MODEL_COUNT))
        return nx_bjt_npn_models[0];

    return nx_bjt_npn_models[index];
}
...
{
    return "Is=" + juce::String(bjt.Is, 2, true)
         + "  Bf=" + juce::String(bjt.Bf, 1)
...
juce::String formatIbSweepSummary(const IbSweepParams& params)
{
    ...
    return juce::String(safe.count) + " steps  |  Ib "
         + formatCurrentWithUnit(safe.minAmps) + " .. " + formatCurrentWithUnit(maxIb)
         + "  |  step " + formatCurrentWithUnit(safe.stepAmps);
}
...
std::vector<float> buildIbSweepValues(const IbSweepParams& params)
{
    ...
    for (int i = 0; i < safe.count; ++i)
        values.push_back(safe.minAmps + safe.stepAmps * static_cast<float>(i));
                                                                      ...
                                                                      float vceMaxVolts)
{
...

    {
        ...
        if (cache.valid && cache.model == model && cache.circuit == circuit
            && cache.numPoints == numPoints && juce::approximatelyEqual(cache.vceMaxVolts, vceMaxVolts)
            && ibValuesEqual(cache.ibValues, ibValues))
        {
...
CurrentDisplayUnit chooseCurrentDisplayUnit(float maxAbsAmps) noexcept
{
    ...
    if (absAmps >= 1.0e-3f)
        return CurrentDisplayUnit::Milliamps;
    if (absAmps >= 1.0e-6f)
        return CurrentDisplayUnit::Microamps;
    if (absAmps >= 1.0e-9f)
        return CurrentDisplayUnit::Nanoamps;
                             ...
                             const DcOperatingPoint& qPoint) noexcept
{
    ...
    if (qPoint.vbe >= 0.0f)
        range.maxX = juce::jmax(range.maxX, qPoint.vbe);

    const float qCurrent = kind == CurveKind::IbVsVbe ? qPoint.ib : qPoint.ic;
    if (qCurrent > 0.0f)
        range.maxY = juce::jmax(range.maxY, qCurrent);
}
...
DcOperatingPoint computeStaticOperatingPoint(nx_bjt_npn_model_e model, CircuitKind circuit)
{
    switch (circuit)
    {
        ...
        {
            nx_bjt_common_emitter_config_t config{};
            if (!commonEmitterFixedConfig(model, config))
                return {};
            const auto c = bjt_common_emitter_design_core(kSampleRate, &config);
            if (!isCommonEmitterDcValid(config, c))
                return {};
            return fillOperatingPoint(c.vb_dc - c.ve_dc,
                                      c.vc_dc - c.ve_dc,
                                      ibAtCommonEmitterDc(c),
                                      icAtCommonEmitterDc(c));
        }
        case CircuitKind::Follower:
        {
            nx_bjt_follower_config_t config{};
            ...
            const auto c = bjt_follower_design_core(kSampleRate, &config);
            const double vbe = c.vb_dc - c.ve_dc;
            const double vce = config.vcc - c.ve_dc;
            const double vbc = c.vb_dc - config.vcc;
            double ib = 0.0;
            double ic = 0.0;
            nx_bjt_em_currents_f64(c.bjt_Is, c.bjt_Vt, c.bf, c.br, vbe, vbc, &ib, &ic);
                                                      ...
                                                      float vceMaxVolts)
{
    ...
    if (!commonEmitterFixedConfig(model, baseConfig))
        return overlay;

    const bjt_common_emitter_coeffs_t baseCoeffs =
        bjt_common_emitter_design_core(kSampleRate, &baseConfig);
    ...
    for (const auto& family : families)
    {
        for (const auto& p : family)
            maxFamilyIc = juce::jmax(maxFamilyIc, p.second);
    }
    ...
    {
        auto q = fillOperatingPoint(baseCoeffs.vb_dc - baseCoeffs.ve_dc,
                                    baseCoeffs.vc_dc - baseCoeffs.ve_dc,
                                    ibAtCommonEmitterDc(baseCoeffs),
                                    icAtCommonEmitterDc(baseCoeffs));
        if (q.valid)
        {
            qVce = q.vce;
            // Snap Ic onto the load line at this Vce so Q sits on the drawn dashed line.
    ...
    const double loadLineEndVce = juce::jmax(static_cast<double>(vceMaxVolts), static_cast<double>(qVce));
    overlay.loadLines.resize(1);
    overlay.loadLines[0] = buildLoadLineSegment(line, 0.0, loadLineEndVce);
                                                            ...
                                                            const AxisRange& range)
{
    ...
    if (!clipTest(-dx, x0 - xmin) || !clipTest(dx, xmax - x0) || !clipTest(-dy, y0 - ymin)
        || !clipTest(dy, ymax - y0) || u0 > u1)
        return out;
...
{
    for (auto& segment : overlay.loadLines)
        segment = clipSegmentToAxisRange(segment, range);
}
...
{
    if (std::abs(volts) < 0.01f)
        return juce::String(volts * 1000.0f, 0) + " mV";
...
juce::String formatCurrentTick(float amps)
{
    ...
    if (absAmps >= 1.0e-3f)
        return formatCurrentTick(amps, CurrentDisplayUnit::Milliamps);
    if (absAmps >= 1.0e-6f)
        return formatCurrentTick(amps, CurrentDisplayUnit::Microamps);
    if (absAmps >= 1.0e-9f)
        return formatCurrentTick(amps, CurrentDisplayUnit::Nanoamps);
...
juce::String formatCurrentWithUnit(float amps)
{
    const float absAmps = std::abs(amps);
    if (absAmps >= 1.0e-3f)
        return formatCurrentTick(amps, CurrentDisplayUnit::Milliamps) + " mA";
    if (absAmps >= 1.0e-6f)
        return formatCurrentTick(amps, CurrentDisplayUnit::Microamps) + " uA";
    if (absAmps >= 1.0e-9f)
        return formatCurrentTick(amps, CurrentDisplayUnit::Nanoamps) + " nA";
