...
{
  return a.count == b.count && juce::approximatelyEqual(a.minAmps, b.minAmps) &&
...
BjtCurvePanel::~BjtCurvePanel()
{
  ...
  cancelPendingUpdate();
  for (int i = 0; i < 200 && rebuildInFlight_.load(std::memory_order_acquire); ++i) juce::Thread::sleep(1);
}

void BjtCurvePanel::setModel(bjt_curve::BjtModelKind model)
{
...
void BjtCurvePanel::setIbSweep(const bjt_curve::IbSweepParams& params)
{
  ...
  next.sanitise();
  if (ibSweepEqual(ibSweep_, next)) return;
...
void BjtCurvePanel::setVceMaxVolts(float vceMaxVolts)
{
  ...
  if (juce::approximatelyEqual(vceMaxVolts_, next)) return;
...
void BjtCurvePanel::setVbeMaxVolts(float vbeMaxVolts)
{
  ...
  if (juce::approximatelyEqual(vbeMaxVolts_, next)) return;
...
void BjtCurvePanel::setCurrentMaxAmps(float currentMaxAmps)
{
  ...
  if (juce::approximatelyEqual(currentMaxAmps_, next)) return;
...
void BjtCurvePanel::handleAsyncUpdate()
{
  ...
  if (rebuildInFlight_.exchange(true, std::memory_order_acq_rel)) return;
  ...
  juce::Thread::launch(
      [safe, params]()
      {
        ...
        juce::MessageManager::callAsync(
            [safe, result = std::move(result)]() mutable
            {
...
BjtCurvePanel::RebuildResult BjtCurvePanel::computeRebuild(const RebuildParams& params)
{
  ...
  result.useFamilyPlot =
      params.curveKind == bjt_curve::CurveKind::IcVsVce && params.circuit == bjt_curve::CircuitKind::CommonEmitter;
  result.unsupportedOutputCurve =
      params.curveKind == bjt_curve::CurveKind::IcVsVce && params.circuit != bjt_curve::CircuitKind::CommonEmitter;

  if (result.useFamilyPlot)
  {
    result.curveFamilies = bjt_curve::buildIcVsVceFamily(params.model, params.circuit, result.ibValues,
                                                         kFamilyNumPoints, params.vceMaxVolts);
    result.dcOverlay = bjt_curve::buildCommonEmitterDcLoadLineOverlay(params.model, result.ibValues,
                                                                      result.curveFamilies, params.vceMaxVolts);
  ...
  else
  {
    ...
    result.samples = bjt_curve::buildCurve(params.model, params.circuit, params.curveKind, ib, kTransferNumPoints,
                                           params.vceMaxVolts, params.vbeMaxVolts);
    ...
    bjt_curve::applyTransferAxisLimits(result.axisRange, params.curveKind, params.vbeMaxVolts, params.currentMaxAmps,
  ...
  // PNP devices conduct with negative Vbe/Vce, so mirror the X axis onto the
  // negative side (curves are already generated on the negative axis).
  if (bjt_curve::isPnp(params.model))
  {
    const float xMin = result.axisRange.minX;
    const float xMax = result.axisRange.maxX;
    result.axisRange.minX = -xMax;
    result.axisRange.maxX = -xMin;
  }
...
void BjtCurvePanel::applyRebuildResult(RebuildResult&& result)
{
  ...
  if (result.useFamilyPlot || result.unsupportedOutputCurve)
  {
    ...
    if (result.unsupportedOutputCurve) curveControl.clearSecondaryCustomCurve();
  }
  else
  {
    ...
    yAxis.formatTick = [currentUnit](float value) { return bjt_curve::formatCurrentTick(value, currentUnit); };
...
float BjtCurvePanel::dataToX(float x) const noexcept
{
  ...
  if (area.getWidth() <= 0.0f || axisRange_.maxX <= axisRange_.minX) return area.getX();
...
{
  const auto area = getPlotArea();
  if (area.getHeight() <= 0.0f || axisRange_.maxY <= axisRange_.minY) return area.getY();
...
void BjtCurvePanel::paintFamilyCurves(juce::Graphics& g)
{
  ...
  for (int i = 0; i < total; ++i)
  {
    ...
    atom::buildVerticallyClippedPolylinePaths(curveFamilies_[static_cast<size_t>(i)], axisRange_.minY, axisRange_.maxY,
                                              mapPoint, strokePath);
  ...
  if (!dcOverlay_.loadLines.empty() && dcOverlay_.loadLines.front().size() >= 2)
  {
    ...
    for (size_t j = 1; j < segment.size(); ++j) loadLinePath.lineTo(mapPoint(segment[j].first, segment[j].second));
  ...
  if (dcOverlay_.qPoint.valid)
  {
    ...
    if (labelX + tw > plotArea.getRight() - 4.0f) labelX = x - tw - 10.0f;
    if (labelY < plotArea.getY() + 2.0f) labelY = y + 10.0f;
  ...
  for (const float xTick : bjt_curve::buildNiceTicks(axisRange_.minX, axisRange_.maxX))
  {
    ...
    g.drawText(label, juce::Rectangle<float>(x - tw * 0.5f, plotArea.getBottom() + 4.0f, tw, 12.0f),
  ...
  for (const float yTick : bjt_curve::buildNiceTicks(axisRange_.minY, axisRange_.maxY))
  {
    ...
    const float y = dataToY(yTick);
    const float tw = g.getCurrentFont().getStringWidthFloat(label);
    g.drawText(label, juce::Rectangle<float>(4.0f, y - 6.0f, juce::jmax(48.0f, tw + 4.0f), 12.0f),
               juce::Justification::centredRight);
  }

  g.drawText(xLabel_, plotArea.withY(plotArea.getBottom() + 18.0f).withHeight(14.0f), juce::Justification::centred);
  g.drawText(yLabel_, juce::Rectangle<float>(4.0f, plotArea.getY() - 18.0f, 56.0f, 12.0f),
  ...
  {
    g.setColour(colors.label);
    g.drawText("--- DC load line", juce::Rectangle<float>(legendX + 14.0f, legendY - 1.0f, 100.0f, 12.0f),
               juce::Justification::centredLeft);
    legendY += 14.0f;
  }
  ...
  if (dcOverlay_.qPoint.valid)
  {
    ...
    g.setColour(colors.label);
    g.drawText("Q (bias)", juce::Rectangle<float>(legendX + 14.0f, legendY - 1.0f, 76.0f, 12.0f),
               juce::Justification::centredLeft);
    legendY += 14.0f;
  }
  ...
  for (int i = 0; i < total; ++i)
  {
    ...
    g.drawText("Ib " + bjt_curve::formatCurrentWithUnit(ibValues_[static_cast<size_t>(i)]),
               juce::Rectangle<float>(legendX + 14.0f, legendY - 1.0f, 76.0f, 12.0f), juce::Justification::centredLeft);
    legendY += 14.0f;
  }
}
...
void BjtCurvePanel::paintTransferQPoint(juce::Graphics& g)
{
  ...
  if (!q.valid || !curveControl.isVisible()) return;
  ...
  if (plotArea.isEmpty() || axisRange_.maxX <= axisRange_.minX || axisRange_.maxY <= axisRange_.minY) return;
  ...
  float labelX = x + 10.0f;
  float labelY = y - 16.0f;
  if (labelX + tw > plotArea.getRight() - 4.0f) labelX = juce::jmax(plotArea.getX() + 4.0f, x - tw - 10.0f);
  if (labelY < plotArea.getY() + 2.0f) labelY = juce::jmin(plotArea.getBottom() - 16.0f, y + 10.0f);
  g.drawText(qLabel, juce::Rectangle<float>(labelX, labelY, tw + 2.0f, 14.0f), juce::Justification::centredLeft);
}
...
void BjtCurvePanel::resized() { curveControl.setBounds(getLocalBounds()); }
