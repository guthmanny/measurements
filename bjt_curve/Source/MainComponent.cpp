...
MainComponent::MainComponent()
{
  ...
  modelBox.addItem("Ideal PNP", 4);
  modelBox.addItem("2N3906", 5);
  modelBox.addItem("AC128 (PNP)", 6);
...
{
  if (kind == lastTransferKind_) return;
...
void MainComponent::applyTheme()
{
  ...
  if (curvePanel != nullptr) curvePanel->applyTheme(themeColors);
...
void MainComponent::updateCurveViews()
{
  ...
  const bool showTransferAxisControls = kind == bjt_curve::CurveKind::IcVsVbe || kind == bjt_curve::CurveKind::IbVsVbe;

  if (showTransferAxisControls) syncTransferCurrentSlider(kind);

  for (auto* control : {static_cast<juce::Component*>(&ibCountLabel), static_cast<juce::Component*>(&ibCountSlider),
  ...
  for (auto* control : {static_cast<juce::Component*>(&vbeMaxLabel), static_cast<juce::Component*>(&vbeMaxSlider),
...
bjt_curve::BjtModelKind MainComponent::getModelFromSelection() const
{
  switch (modelBox.getSelectedId())
  {
    case 2:
      return bjt_curve::BjtModelKind::Npn3904;
    case 3:
      return bjt_curve::BjtModelKind::Npn2222;
    case 4:
      return bjt_curve::BjtModelKind::IdealPnp;
    case 5:
      return bjt_curve::BjtModelKind::Pnp3906;
    case 6:
      return bjt_curve::BjtModelKind::Ac128;
    default:
      return bjt_curve::BjtModelKind::IdealNpn;
  }
}
...
void MainComponent::resized()
{
  ...
  const auto kind = getCurveKindFromSelection();
  const bool showIbSweepControls =
      kind == bjt_curve::CurveKind::IcVsVce && getCircuitFromSelection() == bjt_curve::CircuitKind::CommonEmitter;
  const bool showTransferAxisControls = kind == bjt_curve::CurveKind::IcVsVbe || kind == bjt_curve::CurveKind::IbVsVbe;
  ...
  if (curvePanel != nullptr) curvePanel->setBounds(area.reduced(0, 2));
}
