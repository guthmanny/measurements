/*
  Custom Standalone entry point for Moog Ladder.
...
void ensureEffectInputUnmuted(juce::StandalonePluginHolder& holder)
{
  ...
  if (auto* props = holder.settings.get()) props->setValue("shouldMuteInput", false);
}
...
void restartCurrentAudioDevice(juce::AudioDeviceManager& deviceManager)
{
  ...
  if (setup.inputDeviceName.isEmpty() && setup.outputDeviceName.isEmpty()) return;
...
{
 public:
  NativeStandaloneFilterWindow(const juce::String& title, juce::Colour backgroundColour,
      ...
      : StandaloneFilterWindow(title, backgroundColour, settingsToUse, takeOwnershipOfSettings,
                               preferredDefaultDeviceName, preferredSetupOptions, constrainToConfiguration,
                               autoOpenMidiDevices)
  {
    ...
    for (int i = getNumChildComponents(); --i >= 0;)
      if (auto* button = dynamic_cast<juce::TextButton*>(getChildComponent(i)))
        if (button->getButtonText() == "Options") button->setVisible(false);

    if (auto* holder = juce::StandalonePluginHolder::getInstance())
    {
      ensureEffectInputUnmuted(*holder);
      effect_jack::ensureJackUsesCaptureInput(getDeviceManager(), juce::CharPointer_UTF8(JucePlugin_Name));

      juce::Component::SafePointer<NativeStandaloneFilterWindow> safeWindow(this);
      juce::Timer::callAfterDelay(300,
                                  [safeWindow]()
                                  {
                                    if (safeWindow == nullptr) return;
 ...
 private:
  void syncFixedWindowSize()
  {
    ...
    if (auto* c = getConstrainer()) c->setSizeLimits(expectedW, expectedH, expectedW, expectedH);
    ...
    auto target = (lastStableBounds.isEmpty() ? getScreenBounds() : lastStableBounds).withSize(expectedW, expectedH);
...
class MoogLadderStandaloneApp : public juce::JUCEApplication
{
 public:
  MoogLadderStandaloneApp()
  {
  ...
  {
    if (mainWindow != nullptr) mainWindow->getPluginHolder()->savePluginState();
 ...
 private:
  juce::StandaloneFilterWindow* createWindow()
  {
    ...
    return new NativeStandaloneFilterWindow(
        getApplicationName(),
        juce::LookAndFeel::getDefaultLookAndFeel().findColour(juce::ResizableWindow::backgroundColourId),
...
juce::JUCEApplicationBase* juce_CreateApplication() { return new MoogLadderStandaloneApp(); }
